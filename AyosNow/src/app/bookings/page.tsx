    ...
    >
      {!loadFailed ? (
        <article className="rounded-3xl border border-teal-100 bg-teal-50 px-5 py-4 text-slate-800">
          <p className="text-sm font-bold text-teal-800">{text.bookingsSourceLiveTitle}</p>
          <p className="mt-2 text-sm leading-6 text-slate-700">
            {text.bookingsSourceLiveDescription}
          </p>
        </article>
      ) : null}
      ...
      {loadFailed ? (
        <article className="rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 text-amber-950">
          <p className="text-sm font-bold text-amber-900">
            {locale === "fil" ? "Hindi ma-load ang booking data." : "Could not load booking data."}
          </p>
