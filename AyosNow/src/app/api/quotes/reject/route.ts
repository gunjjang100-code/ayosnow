...
import { rejectQuoteForCustomer } from "@/lib/quotes/service";
...
export async function POST(request: NextRequest) {
  ...
  if (roleState.role !== "customer") {
    return NextResponse.json(
      { error: "Quotes can only be rejected by customer accounts." },
      { status: 403 },
    );
  }
  ...
  try {
    const result = await rejectQuoteForCustomer({
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Could not reject the quote.",
      },
