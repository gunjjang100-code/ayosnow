  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || !result?.service) {
        throw new Error(
          locale === "fil"
            ? "Hindi na-save ang service. Subukan ulit."
  ...
  const handleToggle = async (targetId: string) => {
    ...
    try {
      ...
      if (!response.ok || !result?.service) {
        throw new Error(
          locale === "fil"
            ? "Hindi nabago ang service status. Subukan ulit."
            : "Could not change the service status. Please try again.",
        );
      }
