...
function formatPhp(amount: number, locale: Locale) {
  ...
  if (locale === "ko") {
    return `PHP ${value.toLocaleString("ko-KR")}`;
  }
...
export async function listDashboardStatsForUser(params: {
    ...
    return [
      {
        ...
        helper:
          params.locale === "ko"
            ? "현재 고객 계정의 pending, accepted, in-progress 예약 수입니다."
            : params.locale === "fil"
      ...
      },
      {
        ...
        helper:
          params.locale === "ko"
            ? "요청서 작성 후 첫 견적이 들어오기까지 걸린 평균 시간입니다."
            : params.locale === "fil"
  ...
  if (params.role === "tradesman") {
    ...
    return [
      ...
      },
      {
        label:
          params.locale === "ko"
            ? "새 견적 요청"
            : params.locale === "fil"
              ? "Bagong quote request"
              : "New quote requests",
        value: String(matchingOpenRequestsCount),
        helper:
          params.locale === "ko"
            ? "내 기술 카테고리와 맞는 OPEN 요청 수입니다."
            : params.locale === "fil"
      ...
      },
      {
        label:
          params.locale === "ko"
            ? "이번 주 예약"
            : params.locale === "fil"
              ? "Bookings ngayong linggo"
              : "Bookings this week",
        value: String(bookingsThisWeekCount),
        helper:
          params.locale === "ko"
            ? "이번 주 일정으로 잡힌 예약 건수입니다."
            : params.locale === "fil"
      ...
      },
      {
        ...
        helper:
          params.locale === "ko"
            ? "견적을 처음 제출하며 차감된 40 PHP 수수료의 누적 합계입니다."
            : params.locale === "fil"
  ...
  return [
    {
      label: params.locale === "ko" ? "진행 중 예약" : params.locale === "fil" ? "Aktibong booking" : "Active bookings",
      value: String(activeBookingsCount),
      helper:
        params.locale === "ko"
          ? "플랫폼 전체 기준 진행 중 예약 수입니다."
          : params.locale === "fil"
