...
interface AdminCategoryManagerText {
  ...
  adminCategoryNameKoError: string;
  ...
  adminCategoryDescriptionKoError: string;
...
export function AdminCategoryManager({
  ...
  const validateForm = (value: CategoryFormState) => {
    ...
    if (value.nameKo.trim().length < 2) {
      nextErrors.nameKo = text.adminCategoryNameKoError;
    }
    ...
    if (value.descriptionKo.trim().length < 10) {
      nextErrors.descriptionKo = text.adminCategoryDescriptionKoError;
    }
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      descriptionKo: formState.descriptionKo,
  ...
  const handleStartEdit = (item: AdminCategoryItem) => {
    ...
    setFormState({
      nameKo: item.nameKo ?? item.name ?? getLocalizedAdminCategoryName(item, "ko"),
      ...
      descriptionKo:
        item.descriptionKo ??
        item.description ??
        getLocalizedAdminCategoryDescription(item, "ko"),
      descriptionFil:
        item.descriptionFil ??
        ...
        >
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-3">
            <label className="grid gap-2 text-sm font-semibold text-slate-800">
              <span>{text.adminCategoryNameKoLabel}</span>
              <input
                value={formState.nameKo}
                onChange={handleInputChange("nameKo")}
                ...
                placeholder="예: 홈 인테리어"
              />
              {fieldErrors.nameKo ? (
                <span className="text-xs font-semibold text-rose-600">{fieldErrors.nameKo}</span>
              ) : null}
            </label>
            ...
            <label className="grid gap-2 text-sm font-semibold text-slate-800 xl:col-span-3">
              <span>{text.adminCategoryDescriptionKoLabel}</span>
              <textarea
                value={formState.descriptionKo}
                onChange={handleInputChange("descriptionKo")}
                rows={4}
                className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-900 outline-none transition focus:border-teal-500"
                placeholder="예: 부분 리모델링, 벽지, 조명, 가구 배치 상담을 연결합니다."
              />
              <span className="text-xs leading-5 text-slate-500">
                {text.adminCategoryDescriptionHint}
              </span>
              {fieldErrors.descriptionKo ? (
